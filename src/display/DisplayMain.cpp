...
void DisplayMain::render()
{
	...
	else
	{
		context.displayInfo.ir_event = 0;
		context.displayInfo.ir_key = 0;
		sprintf(txt, "%04d.%02d.%02d", context.displayInfo.datetime.year,
				context.displayInfo.datetime.month,
				context.displayInfo.datetime.day);
		context.lcd.center(0, txt);
		memset(txt, 0, 17);
		sprintf(txt, "%02d:%02d:%02d", context.displayInfo.datetime.hour,
				context.displayInfo.datetime.minute,
